...
class AddLentScreen extends StatefulWidget {
  final Lent? existingLent;
  const AddLentScreen({super.key, this.existingLent});
  bool get isEditing => existingLent != null;
...
class _AddLentScreenState extends State<AddLentScreen> {
  ...
  void initState() {
    super.initState();
    final existing = widget.existingLent;
  ...
  Future<void> _submit() async {
    ...
    await context.read<LentCubit>().save(lent);

    if (!mounted) return;
    Navigator.of(context).pop(true);
    AppSnackbar.showSuccess(context, widget.isEditing ? 'Updated' : 'Added to Lent');
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _sectionLabel(colors, 'RETURN BY'),
                const SizedBox(height: 8),
                GestureDetector(
                  onTap: _pickDate,
                  child: Container(
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    maxLines: 2,
                    decoration: InputDecoration(
                      hintText: 'e.g. For his bus ticket',
                      hintStyle: AppTextStyles.body(colors.textSecondary),
                      border: InputBorder.none,
                      contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 14),
                    ),
...
Future<bool?> openAddLentSheet(BuildContext context, {Lent? existingLent}) {
  return showModalBottomSheet<bool>(
    ...
    builder: (context) => FractionallySizedBox(
      heightFactor: 0.95,
      child: AddLentScreen(existingLent: existingLent),
    ),
  );
}