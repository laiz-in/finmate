...
class ExpensesBloc extends Bloc<ExpensesEvent, ExpensesState> {
  ...
  final List<ExpensesEntity> _allExpenses = []; // Store all fetched expenses
  int currentPage = 0; // For lazy loading, we track the page number
  ...
  void _onFetchAllExpenses(FetchAllExpensesEvent event, Emitter<ExpensesState> emit) async {
    ...
    currentPage = 0; // Reset the pagination
    _hasMore = true;
  ...
  final filteredExpenses = _filterExpenses(_allExpenses);

  emit(ExpensesLoaded(filteredExpenses, hasMore: _hasMore));
}
...
List<ExpensesEntity> _filterExpenses(List<ExpensesEntity> expenses) {
  if (_searchQuery.isEmpty) {
    ...
    return expenses
        .where((expense) => expense.spendingDescription
            .toLowerCase()
            .contains(_searchQuery.toLowerCase())) // Filter by description
        .toList();
  }
}


 // last 15 days total expenses
